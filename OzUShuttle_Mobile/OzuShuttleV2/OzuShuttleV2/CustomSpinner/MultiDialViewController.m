...
#define DIAL_OFFSET_X               20
#define DIAL_OFFSET_Y               0
#define DIAL_WIDTH                  120
#define DIAL_HEIGHT                 140
...
- (void)viewDidLoad {
    ...
    NSArray *numbers = [NSArray arrayWithObjects:@"Altunizade", @"Çekmeköy", nil];
    int dialCount = 0;
    ...
    self.dial2 = [[[DialController alloc] initWithDialFrame:CGRectMake(DIAL_OFFSET_X*2 + dialCount++ * DIAL_WIDTH, DIAL_OFFSET_Y, DIAL_WIDTH, DIAL_HEIGHT) strings:numbers] autorelease];
...
- (void)dialController:(DialController *)dial didSnapToString:(NSString *)value {    
    if (!self.dial1.isSpinning && !self.dial2.isSpinning) {
        NSString *selectedString = [NSString stringWithFormat:@"%@%@", self.dial1.selectedString, self.dial2.selectedString];
